	...
	const sceneInfo = [
		{
			...
			objs: {
				...
				logo: document.querySelector('#scroll-section-0 .logo'),
	...
	function playAnimation() {
		...
		switch (currentScene) {
			case 0:
				...
				if (scrollRatio <= 0.4) {
					objs.logo.style.width = `${calcValues(values.logo_width_in, currentYOffset)}vw`;
					objs.logo.style.transform = `translate(${calcValues(values.logo_translateX_in, currentYOffset)}%, -50%)`;
				} else {
					objs.logo.style.width = `${calcValues(values.logo_width_out, currentYOffset)}vw`;
					objs.logo.style.transform = `translate(${calcValues(values.logo_translateX_out, currentYOffset)}%, -50%)`;
				}
